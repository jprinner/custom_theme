...
// register shortcode
add_shortcode('city_population', 'city_population_shortcode');
...
function internet_types_availability_shortcode($attrs) { 
...
function max_residential_download_speed_in_city_shortcode() { 
	if (get_page_template('template-paid-city-lander')){
		...
		return max(array_column($zip_results_arr['internet'], 'download_speed'));
	} else {
...
function max_residential_download_speed_provider_in_city_shortcode() { 
	if (get_page_template('template-paid-city-lander')){
		...
		$download_speed_arr_col = array_column($zip_results_arr['internet'], 'download_speed');
        $download_speed_arr_col = array_search(max($download_speed_arr_col), $download_speed_arr_col);
        return $zip_results_arr['internet'][$download_speed_arr_col]['name'];
	} else {
...
//syncs the BD API for a top city
function bd_api_sync_shortcode() { 
	$html ='';
	$html .= '
	<form action="" method="post">
		<label for="city">City:</label><br>
		<input type="text" id="city" name="city" required><br>
		<label for="state">State (in abbreviated form, e.g. "CO"):</label><br>
	...
	if(isset($_POST["submit"])) {
	    ...
	    if (!empty($api)){
			...
			if ($return){
				$return = $city.', '.$state.' has been successfully synced';
			}
	    } else {
	    	$return = "Something went wrong, please check that the city and state names are correct and try again";
	    }
	    $html = $return.'<br>'.$html;
	} 
	return $html;
}	
add_shortcode('bd_api_sync', 'bd_api_sync_shortcode'); 
...
function zip_to_city_redirect_shortcode() { 
    ...
    <form action="" class="zip_to_city_search_form search_wrap">
        <input type="number" class="zip_to_city_search_input" id="zip" name="zip" minlength="5" maxlength="5" placeholder="Search by Zip Code" pattern="\d*"/>
