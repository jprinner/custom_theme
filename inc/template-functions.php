...
function custom_scripts() {
   ...
   // wp_enqueue_script( 'appzi', 'https://w.appzi.io/w.js?token=jA8Qg', null, null, true );
    ...
    if (!$is_programmatic_city_page && !is_page('zip-search') && (!is_singular('locations') || is_page_template())) :
...
//function to get ctas (url and text) for a given provider
    ...
    if ($provider_id):
        ...
        if($partner){
            ...
            foreach($campaign as $key => $camp) {
                ...
                if($camp['campaign_name'] == $provider_id){
                    ...
                    if($type_of_partnership == 'call_center'){
                        ...
                        $cta_arr['cta_text'] = '<span class="material-icons">call</span>'.$camp['call_center'];
                        ...
                        if ($camp['primary_conversion_method'] == 'call_center'){
                            ...
                            switch ($style){
                                case 'text-below-button-centered':
                                    ...
                                    break;
                                case 'text-below-button-left':
                                    $cta_arr['cta_text'] = '<span class="material-icons">call</span>'.$camp['call_center'];
                                    $cta_arr['cta_text2'] = '<p class="mt-2 mb-0 tel-link text-left font-weight-bold">Order Online</p>';
                                    break;
                                default:
                                    $cta_arr['cta_text'] = '<span class="material-icons">call</span>'.$camp['call_center'];
                            ...
                            switch ($style){
                                ...
                                case 'text-below-button-left':
                                    $cta_arr['cta_text'] = 'Order Online';
                                    $cta_arr['cta_text2'] = '<p class="mt-2 mb-0 text-left small-text">Call to order:<span class="tel-link font-weight-bold"> '.$camp['call_center'].'</span></p>';    
                                    break;
                                default:
                                    $cta_arr['cta_text'] = 'Order Online';
                                    $cta_arr['cta_text2'] = '<span class="material-icons">call</span>'.$camp['call_center'];
                            }
                            $cta_arr['cta_link'] = $camp['digital_tracking_link'];
            ...
            if (get_field('display_website_url_or_phone_number', $provider_id) == 'phone'){
                ...
                $cta_arr['cta_type'] = 'call_center';
                $cta_arr['cta_text'] = '<span class="material-icons">call</span>'.$phone;
...
add_action( 'admin_footer', 'remove_post_meta_boxes' );

function remove_post_meta_boxes() 
{
    if( isset( $_GET['post'] ) && $_GET['post'] == '13866' ) 
    {
        ?>
        <script>
            $('body').addClass('for-toc-removal');
            $('.for-toc-removal #ez-toc').hide();
        </script>
        <?php
    }
}