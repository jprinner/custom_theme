...
//check if we are on provider's page, if not we disregard the hide cta toggle
if ($post->ID !== $id){
    $hide_cta = false;
}
...
if($id) {
    ...
    $cta_text = $cta_link = $cta_text2 = $cta_link2 = $zip_popup_class = $data_att = '';

    if (!$hide_cta && $button_type != 'popup'):
        if($partner){
        ...
        $cta_text = ($button_text) ? $button_text : 'Check Availability'; 
        $cta_link = '#';
        $rand = rand();
    ...
    <div class="thoughts-container">
        ...
        <div class="provider-info col col-sm-6 col-md-4 p-3">
            ...
            <div class="cta-container flex-column align-items-center">
                <?php
                    if(!empty($cta_text)) echo '<a href="'.$cta_link.'" class="cta_btn '.$zip_popup_class.'" '.$target.' onclick="' . $datalayer . '" '.$data_att.'>'.$cta_text.'</a>';
