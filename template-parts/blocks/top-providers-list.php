...
if($top_providers){
    ...
    foreach($top_providers as $key => $provider):
        ...
        if ($provider['provider_type'] == 'streaming'){

            $provider_id = $provider['provider_streaming'];
            $provider['provider_features'] = $provider['provider_features_streaming'];
            $cta_html = '<a href="'.get_field('link', $provider_id).'" class="cta_btn w-100" target="_blank">View Plans</a>';
            ...
            $cta_html = '<a href="#" class="cta_btn w-100 zip-popup-btn" data-toggle="modal" data-target="#zipPopupModal-'.$rand.'">Check Availability</a>';
        }
