                    ...
                    if (isset($provider['Products']) && is_array($provider['Products'])){
                        ...
                        foreach ($provider['Products'] as $product){
                            ...
                            if ($product['VerticalName'] == 'Internet'){
                                ...
                                //COX hotfix to remove promos if length months is 0
                                if($provider_name == 'COX') {
                                    $initial_price = $product['BasePrice']['InitialAmount'];
                                    $length_months = $product['BasePrice']['InitialAmountLengthMonths'];
                                    $reg_price = $product['BasePrice']['RegularAmount'];

                                    // if($initial_price == 5 || $initial_price == '5.0' ) {
                                    //     error_log('COX ' . $product['ProductName']);
                                    //     error_log(print_r($product['BasePrice'], true));
                                    // }

                                    if(empty($length_months) || $length_months == 0) {
                                        continue;
                                    }
                                    if(empty($reg_price) || $reg_price == 0) {
                                        continue;
                                    }
                                }
