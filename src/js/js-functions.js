...
function populateAffClickID() {
    ...
    const ifLoadedInvoca =   setInterval(function(){ handleInvocaParam(ifLoadedInvoca) }, 511);
    ...
    if(numOfTriesInvoca === 16 ) {  
        ...
        if (typeof Invoca !== undefined ) {            
            ...
            $('a[href*="hsopageurl"]').each((i, el) => {
                ...
                const updatedLink = trackingUrl.replace('{$hsopageurl}', trackingPageURL).replace('$hsopageurl', trackingPageURL).replace('{$hsophonenum}', thisTelNumber[1]).replace('$hsophonenum', thisTelNumber[1]);
                $(el).attr('href', updatedLink);
