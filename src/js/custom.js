...
jQuery(document).ready(function() {
   ...
   // Filters for Explore Topics
    ...
    $('input[name=zip]').on('input', function() {
        $(this).attr('placeholder', 'Search by Zip Code');
    ...
    $(document).on('click', '.see-more-providers', function(e){
        e.preventDefault();
        $(this).parent().parent().find('.provider-hidden').toggle();
...
jQuery(document).ready(function($) {
    ...
    $( ".related_posts_wrap" ).each(function( index ) {
        var countRelatedPosts = $(this).children().children('.related_posts').length;
        if (countRelatedPosts>1){
            $(this).addClass("expand-related-posts");
            $(this).children('.show-less').children('a').click(function () {
            ...
            $(this).removeClass("expand-related-posts");
            $(this).children('.show-less').hide();
            $(this).children().children('.related_posts').css('margin-bottom','0 !important');
        }
      });
})
